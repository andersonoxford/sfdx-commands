echo '-- Powered By: ThinkLP SFDX Commands (0.5) --'
...
scratch() {
  ...
  if [ -z "$PACKAGE_NAME" ]; then
    ...
    if [ -z "$SCRATCH_NAME" ]; then
      ...
      TODAY=$(date +"%Y-%m-%d")
      SCRATCH_NAME="$SCRATCH_NAME"_"$PACKAGE_NAME"_"$TODAY"
      feedback "Creating new scratch org for $PACKAGE_NAME with name $SCRATCH_NAME"

      # Switch package name
      case "$PACKAGE_NAME" in
      core)
        core_scratch "$SCRATCH_NAME" "$DEV_HUB_USERNAME"
        ;;
      audit)
...
help() {
  ...
  feedback 'tlp delete_logs           (To delete ApexLog records in scratch org)'
  feedback '---------------------------------------------------------------------'
}
